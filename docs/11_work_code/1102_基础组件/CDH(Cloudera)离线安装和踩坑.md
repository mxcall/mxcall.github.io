...
## 个人方案
参考业界方案即可, 这里重点记录余下的坑
> 容器内部需要配置crontabs 定时同步时间
```bash
yum install -y crontabs 
systemctl enable  crond 
systemctl start   crond 
systemctl status  crond 
/usr/bin/echo "*/30 * * * * root /usr/sbin/ntpdate cn.ntp.org.cn > /dev/null 2>&1"  >>/etc/crontab  
cat /etc/crontab  
systemctl reload crond && systemctl restart crond  

```

> 容器重启后/etc/hosts文件被重置了, 需要 centos7开机自启动 插入脚本片段
https://www.mmcloud.com/16389.html
```bash
1. /etc/hosts, /etc/resolv.conf和/etc/hostname容器中的这三个文件不存在于镜像,而是存在于于/var/lib/docker/containers/，在启动容器的时候，通过mount的形式将这些文件挂载到容器内部。
因此，如果在容器中修改这些文件的话，修改部分不会存在于容器的top layer，而是直接写入这三个物理文件中。即永久更改;

2. 容器重启后，IP地址改变，会根据当前docker0下的所有节点的IP信息重新建立这3个文件。也就是说，你的修改会被Docker给覆盖掉. 经测试, 包括上面所说的3个(hostname,resolve.conf, hosts)都会被重置为初始化状态。

3. 可以通过docker run命令的–add-host参数来为容器添加host与ip的映射关系。 --add-host=myhostname:10.180.8.1

4. /etc/profile也会碰到修改后, 容器重启后, 没有加载的问题,
在容器实例中 ~/.bashrc 配置文件末尾添加 source /etc/profile，保存退出即可。这样就能够实现重启后自动刷新配置文件，而无需手动操作, 
不过这样有个缺点, 必须用户/程序 登陆后, 才会执行脚本, 而不是开机执行; 另外, 会重复执行, 而不是开机执行一次;
所以最佳写入hosts方案是 修改/etc/rc.local, 再给/etc/rc.local 可执行权限
```
```bash
# 参考(网传除了/etc/rc.local, 还可以/etc/init.d 或 crontab) https://blog.csdn.net/weixin_39533161/article/details/118542778
# 容器内 执行
...
cat >>/etc/hosts<<EOF
# For CDH
192.168.0.240   node240   node240.bridge-to-router
...
```
docker run  --net=bridge-to-router --ip=192.168.0.243 --name node243 --hostname node243 -d  --restart always  --privileged=true  centos:7 /usr/sbin/init && systemctl stop getty@tty1.service && systemctl mask getty@tty1.service
```
